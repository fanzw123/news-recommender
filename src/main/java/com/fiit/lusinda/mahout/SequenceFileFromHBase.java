 ...
 */
package com.fiit.lusinda.mahout;
 ...
 * back in again.
 */
public class SequenceFileFromHBase {
  private static final Log LOG = LogFactory.getLog(SequenceFileFromHBase.class);
  final static String NAME = "export";
  final static String COLUMN_PROPERTY="column";

  /**
   * Mapper.
   */
  static class HBaseExporter
  extends TableMapper<Text, Text> {
    ...
    throws IOException {
      try {
    	  Text key = new Text();
    	  key.set(row.get(), row.getOffset(), row.getLength());
  ...
  throws IOException {
    ...
    if (conf.get(TableInputFormat.SCAN_COLUMNS) != null) {
    	 s.addColumns(conf.get(TableInputFormat.SCAN_COLUMNS));
      }
    LOG.info("verisons=" + versions + ", starttime=" + startTime +
      ", endtime=" + endTime);
    TableMapReduceUtil.initTableMapperJob(tableName, s, HBaseExporter.class, null,
      null, job);
  ...
  private static void usage(final String errorMsg) {
    ...
    System.err.println("   -D " + TableInputFormat.SCAN_COLUMNS + "=<columnName>");
  }
